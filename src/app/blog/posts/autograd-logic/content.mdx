...
The `zeroing out` part occurs right BEFORE each backward pass. The zeroed out gradient for any parameter then gets a total accumulation of its gradients across the network (say it is $W_1.grad$). We will then use this gradient to update the parameter with the help of a small learning rate like this:

So, for instance, if we have a parameter $W_1$, we first zero its gradient (`zero_grad()`), and then accumulate its gradients across the network (say it is $W_1.grad$). We will then use this gradient to update the parameter with the help of a small learning rate like this:

```python
W1.data -= W1.grad * 0.01
```

As soon as we update the parameter's data here, we zero the $W_1.grad$ so that we can do these exact same steps again in the next loop when we once again do `.backward()` (in which case, the $W_1$ parameter will have the newly updated data from the previous optimization, and so on). This continues until we have minimized the loss (without overfitting our data, of course).
...
In this case, the computation graph, from left to right, follows a rough order from x -> y -> L.

Since L in here is the output, the `.backward()` being done to L means we are calculating gradients in each node (maybe we're most likely doing it only for leaf nodes and not the intermediary tensors, so let me assume, for this instance, that the gradient accumulation occurs for leaf nodes only, i.e., the parameters), i.e., the derivative of L with respect to each node, and accumulating gradients for each parameter that occurs more than once across the layers / forward pass.

The topological sort gets applied for the `.backward()` since in the network's normal shape, the flow is a forward flow, i.e., a rough order from x -> y -> L. But the gradient accumulation starts from the output (L here), and in order to follow the right-to-left order (compared to the normal left-to-right order during the forward pass), we do topological sorting of the operations in autograd (flow of the operations from right to left, basically).

To be more specific, the topological sort for my example will be: `L → (y + x) → y → (2*x) → x`. The sort ensures we always apply the chain rule in reverse order of operations creation. This is why my example's gradient for `x` would be `3` (from both `y` and `L` paths).
...
I thought the leaf nodes were x and y. That was incorrect. The leaf node is only x, not y. 

<Callout variant="purple" title="So what is a leaf node?">Often used loosely alongside the term "leaf node tensors", leaf nodes are tensors created directly (like `x` in their example) without being the result of an operation. Intermediates (like `y` and `L`) are not leaf node tensors.</Callout>

Here, I am counting `y` as a parameter as well, but our issue here is that it is not a leaf node but rather an intermediary. But since we need its gradient as well, we need to do `retain_grad=True` so that its gradient also gets tracked by the engine. The `requires_grad=True` is not necessary for intermediaries.

The `requires_grad` allows me to tell the neural network to track the gradients; however, in the flow above, I should do `requires_grad=True` for only `x` (since this is my leaf node tensor) and `retain_grad=True` for `y` (since this is my intermediary tensor).
...
<Text variant="muted" size="xs" className="text-center mt-4 mb-8">A simple illustration of a Directed Acyclic Graph (DAG). <Link href="https://thenlp.space/blog/graph-convolutional-network-for-dummies" target="_blank" className="underline text-purple-500 hover:text-purple-600">Source</Link>.</Text>

Building the graph happens during the forward pass by recording operations. Freeing the graph (lines 178-181 in tensor.py) involves clearing references to allow garbage collection, which was not mentioned. This is important for memory management, especially in large models.
...
Also, the question regarding retaining the grad: do we need to do `retain_grad=True` explicitly only for intermediates or is it also required for leaf nodes? Additionally, I do understand that we need to do `requires_grad=True` if we want any parameter's grad to be tracked, but does this mean that we need to do `requires_grad=True` and `retain_grad=True` for both the leaf node tensors as well as all the intermediate tensors? Or how does it work? I need the answer based on the logic of PyTorch.

**About retaining gradients**: Leaf nodes (like parameters) can just have `requires_grad=True` and their gradients are retained by default. For intermediates, like activation outputs, gradients aren't kept unless `retain_grad()` is called in addition to `requires_grad=True`. The `retain_grad` method sets `_retain_grad` to True, which prevents the warning when accessing `.grad` on non-leaf tensors. So in PyTorch, only leaf nodes retain gradients by default; intermediates need explicit `retain_grad()`.
